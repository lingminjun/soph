 ...
 */
public final class SSNDispatchQueue implements Closeable {
    private static final int CORE_POOL_COUNT = 10;//默认保留线程数，尽量反复利用已有空闲线程，而不是创建线程
    private static final Logger logger = LoggerFactory.getLogger(SSNDispatchQueue.class);

    /**
     * 构造方法
     */
    public SSNDispatchQueue() {
        this(null);
    }
    public SSNDispatchQueue(String name) {
        this(CORE_POOL_COUNT,name);
    }
    public SSNDispatchQueue(int corePoolSize, String name) {
...
//        SingleThreadExecutor
     ...
     * @return
     */
    private static SSNDispatchQueue common_queue = null;
    public static SSNDispatchQueue commonQueue() {
        if (common_queue != null) return common_queue;
        synchronized (SSNDispatchQueue.class) {
    ...
    private static SSNDispatchQueue newTaskQueue(int corePoolSize, String name) {
        return new SSNDispatchQueue(corePoolSize,name);
    }
